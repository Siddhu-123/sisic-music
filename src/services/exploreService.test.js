...
const songs = [
  ...
  { songKey: 'b', artist: 'Bright Band', track: 'Summer Rock', genre: 'Rock', playCount: 1, isDownloaded: true },
...
test('Explore search ranks title and facet matches and filters availability', () => {
  assert.equal(rankedExploreSearch('midnight', songs)[0].songKey, 'a');
  assert.equal(rankedExploreSearch('rock', songs)[0].songKey, 'b');
  assert.deepEqual(filterExploreSongs(songs, { availability: 'offline' }).map(song => song.songKey), ['b']);
});

test('Explore mixes are local smart collections and keep not-downloaded songs visible', () => {
  const result = buildExploreMixes(songs, { likedSongKeys: ['a'] });
  assert.ok(result.mixes.some(mix => mix.id === 'your-taste'));
  assert.ok(result.mixes.some(mix => mix.id === 'not-downloaded'));
