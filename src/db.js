...
export async function syncLibraryToDb(songs) {
  // Run the entire sync (dedup + inserts + updates) in one transaction
  // so useLiveQuery only fires ONCE when everything is done.
  return await db.transaction('rw', db.songs, db.metadata, async () => {
    // Dedup once — check flag so we don't re-scan every refresh
    const dedupDone = await db.metadata.get('dedupDone');
    if (!dedupDone) {
      await deduplicateSongs();
      await db.metadata.put({ key: 'dedupDone', value: true });
    }
    ...
    for (const s of songs) {
      ...
      if (!prev) {
        ...
        existingMap.set(key, { id: newId, ...s });
        added++;
      } else {
        const updates = {};
...
async function deduplicateSongs() {
  ...
  for (const [, dupes] of groups) {
    ...
    for (let i = 1; i < dupes.length; i++) toDelete.push(dupes[i].id);
  }

  if (toDelete.length > 0) {
    await db.songs.bulkDelete(toDelete);
    console.log(`Dedup: removed ${toDelete.length} duplicate rows`);
  }
}
